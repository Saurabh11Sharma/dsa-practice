...
// Reuse test case generators for consistency
function generateCases() {
  ...
  for (const size of sizes) {
    // Sorted
    const tcSorted = generateLargeTestCase(size);
    tcSorted.nums.sort((a, b) => a - b);
    cases.push({
      name: `Sorted (n=${size})`,
      nums: tcSorted.nums,
      target: tcSorted.target,
    });

    // Reverse sorted
    const numsRev = Array.from({ length: size }, (_, i) => size - i);
    cases.push({
      name: `Reverse Sorted (n=${size})`,
      nums: numsRev,
    ...
    tcRand.nums.sort((a, b) => a - b); // twoSumII expects sorted input
    cases.push({
      name: `Random (n=${size})`,
      nums: tcRand.nums,
...
(async () => {
  console.log('Benchmarking twoSumII on various input patterns and sizes...\n');
  console.log(
    'Case'.padEnd(30) +
      ' | ' +
      'ops/sec'.padStart(10) +
    ...
    await new Promise<void>((resolve) => {
      ...
      suite
        .add(input.name, function () {
          twoSumII(input.nums, input.target);
        })
